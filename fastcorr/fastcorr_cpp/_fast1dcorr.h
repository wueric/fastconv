...
py::array_t<T, py::array::c_style | py::array::forcecast> batched_filters_batched_data_correlate(
    ...
    py::buffer_info raw_data_info = raw_data.request();
    ...
    py::buffer_info filter_taps_info = batched_filter_taps.request();
...
#pragma omp parallel for collapse(2)
    for (int64_t b_data = 0; b_data < n_batch_data; ++n_batch_data) {
        for (int64_t b_filter = 0; b_filter < n_batch_filters; ++n_batch_filters) {

            int64_t data_offset_ptr = multichan_data_ptr + b_data * (n_channels_raw_data * n_samples_raw_data);
            TwoDContigArrayWrapper<T> data_wrapper = TwoDContigArrayWrapper<T>(
                    data_offset_ptr,
                    n_channels_raw_data,
                    n_samples_raw_data);

            int64_t kernel_offset_ptr = filter_taps_array + b_filter * (n_channels_filters * n_taps_filters);
            TwoDContigArrayWrapper<T> filter_wrapper = TwoDContigArrayWrapper<T>(
                    kernel_offset_ptr,
                    n_channels_filters,
                    n_taps_filters);

            int64_t write_offset_ptr = output_data_ptr + (b_data * n_batch_filters + b_filter) * n_samples_output;
            OneDContigArrayWrapper<T> output_wrapper = OneDContigArrayWrapper<T>(
                    write_offset_ptr,
                    n_samples_output);

            correlate_accum(data_wrapper, filter_wrapper, output_wrapper);
        }
    }
